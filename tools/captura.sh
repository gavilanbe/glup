...
"$CHROME" --headless=new --disable-gpu --hide-scrollbars --no-first-run --window-size=1280,720 --enable-logging=stderr --v=0 \
  --virtual-time-budget=3000 --screenshot="$OUT" "file://$PWD/index.html?escena=$NAME&t=$T&n=$N&x=$X&guion=$GUION&debug=1&trucos=$TRUCOS${ZOOM:+&z=$ZOOM}" 2>&1 | grep -E 'CONSOLE' | sed 's/.*CONSOLE/CONSOLE/' | head -40
echo "$OUT"
